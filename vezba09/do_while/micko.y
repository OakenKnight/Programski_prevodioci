	...
	: _DO _LBRACKET
		{
			...
			$<i>$=lab_num;
			code("\ndo%d:",lab_num);
		} statement_list _ID _INC _SEMICOLON
		{
			int idx = lookup_symbol($5, VAR|PAR);
        if(idx == NO_INDEX)
					err("'%s' is undeclared",$5);
			code("\n\t\t%s\t$1,",ar_instructions[(get_type(idx)-1)*AROP_NUMBER]);
			gen_sym_name(idx);
			code(",");
			gen_sym_name(idx);
		} _RBRACKET _WHILE _LPAREN rel_exp 
		{
			code("\n\t\t%s\texit%d",opp_jumps[$12],$<i>3);
			code("\n\t\tJMP\tdo%d",$<i>3);
			code("\nexit%d:",$<i>3);
		}_RPAREN _SEMICOLON
	;
while_statement
	: _WHILE 
		...
		_LPAREN rel_exp _RPAREN _LBRACKET
		{
			code("\n\t\t%s\texit%d",opp_jumps[$4],$<i>2);
		} statement_list _ID _INC _SEMICOLON _RBRACKET
		{
